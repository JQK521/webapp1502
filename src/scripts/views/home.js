...
SPA.defineView('home', {
  ...
  console.log(e);    this.myHomeSwiper.slideTo($(e.el).index());
      $('#m-index-container > spa-view > #fixed-menu-body').hide();
    },
    'follow': function (e) {
      // 获得视图公共的home swiper, 跳转到某个slider
      this.myHomeSwiper.slideTo($(e.el).index());
      $('#m-index-container > spa-view > #fixed-menu-body').show();
    },
      ...
      setTimeout(function () {
        ...
        myFS.on('scroll', function(){
          ...
          if( Math.abs(this.y) >= offsetTop - 44 ) {
            if($('#m-index-container > spa-view > #fixed-menu-body').length <= 0){
              $fixedMenu.clone(true)
                .attr({
                  'id': 'fixed-menu-body'
                })
                .appendTo($('#m-index-container > spa-view'));

              $('#m-index-container > spa-view > #fixed-menu-body > input').attr('ms-duplex', 'surname');

              avalon.scan(that.root, vm);
            }
          } else {
            $('#m-index-container > spa-view > #fixed-menu-body').remove();
          }
        });
